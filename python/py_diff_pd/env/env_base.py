...
class EnvBase:
    ...
    # f_ext: either None or a list of size frame_num whose element is of size dofs. Can also be a callable function that takes as inputs the current frame index, q, and v, and returns f_ext for the current timestep.
    ...
    def simulate(self, dt, frame_num, method, opt, q0=None, v0=None, act=None, f_ext=None,
        ...
        if f_ext is None or isinstance(f_ext, Callable):
            # If callable function, we later compute f_ext at each time step based on current q and v.
        ...
        for i in range(frame_num):
            ...
            # Compute forces
            if isinstance(f_ext, Callable):
                sim_f_ext[i] = f_ext(i, q[-1], v[-1])
