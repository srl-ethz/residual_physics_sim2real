...
def voxelize(triangle_mesh_file_name, dx, feather=0.0, normalization_factor=False):
    tri_mesh = trimesh.load(triangle_mesh_file_name)
    assert tri_mesh.is_watertight
    bbx_offset = np.min(tri_mesh.vertices, axis=0)
    tri_mesh.vertices -= bbx_offset
    ...
    # Return the normalization scaling, such that we can use original sizes
    if normalization_factor:
        return voxels, np.max(bbx_extent)
    return voxels