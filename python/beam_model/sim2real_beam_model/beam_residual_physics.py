...
class CantileverResidualPhysics(ResidualPhysicsBase):
    ...
    def train(self, data_path, config):
        ...
        training_set = CantileverDataset(
            training_set_index,
            torch.from_numpy(diffpd_model._q0),
            data_path,
        ...
        if config['fit'] == "SITL":
            ...
            forward_model : LearningFoward = SupervisedLearningForward(diffpd_model, training_set, self.residual_network, self.loss_fn) 
    ...
    def fit_residual_physics(
        ...
        with tqdm(total=self.epochs) as qbar:
            ...
            for epoch in range(self.epochs):
                ...
                elif self.validate_physics and epoch % self.validate_epochs == 0:
                    self.residual_network.to('cpu')
                    val_loss_metrics = validate_main(self._folder, self.validation_set_index, self.residual_network)
                    val_loss = val_loss_metrics['validate_trajectory_error_mean']
                    self.residual_network.to(device)
                else:
